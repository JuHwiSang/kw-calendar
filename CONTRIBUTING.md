...
#### parse_events 환경변수 설정

로컬 Edge Function 개발 시 `supabase/functions/.env` 파일에 아래 값을 추가합니다.
...
- `USE_MOCK_LLM=true`이면 실제 Gemini API 호출 대신 mock 응답을 사용합니다.
- 실제 Gemini API를 호출하려면 `USE_MOCK_LLM=false`로 변경하고, `GEMINI_API_KEY`에 발급받은 API Key를 입력합니다.
- `supabase/functions/.env` 파일은 로컬 개발용 파일이므로 Git에 커밋하지 않습니다.
